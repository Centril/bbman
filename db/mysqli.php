...
class DB
{
	...
	function connect($errormode = 'print') // connect to the database
	{
		if($errormode === 'return')
		{
			$ext_test = ( function_exists('mysqli_connect') ? '' : -1);
			if($ext_test === -1) return -1;
			return (@mysqli_connect($this->host,$this->user,$this->password,$this->database) ? 1 : 0);
		}
		
		$this->conn['server'] = $this->conn['database'] = mysqli_connect($this->host,$this->user,$this->password,$this->database) or $this->error('Can\'t connect to the database or server');
	}

	function close() // close the database connection
	{
		$this->query = NULL;
		mysqli_close($this->conn['server']);
	}
	...
	function query($query,$queryname,$overwrite = 'no') // run a query
	{
		...
		{
			$this->query[$queryname] = $this->currMethod = mysqli_query($this->conn['server'],$query) or $this->error("Invalid query-{$queryname}");
	...
	{
		$this->currMethod = mysqli_fetch_row($this->query[$queryname]);
		return $this->currMethod;
	}
	...
	function fetch_assoc($queryname,$load = 'yes') // fetch a row as an hash
	{
		...
		{
			$this->currMethod = mysqli_fetch_assoc($queryname);
			return $this->currMethod;
		}
		else
		{
			$this->currMethod = mysqli_fetch_assoc($this->query[$queryname]);
			return $this->currMethod;
		}
	}
	...
	{
		$this->currMethod = mysqli_num_rows($this->query[$queryname]) or $this->error('Can\'t get the number of rows from query : '.$queryname);
	...
	function ALTER_TABLE($table,$action,$column,$type = '',$length = '',$newcolumn = '') // add,edit,delete columns from a table
	{
		...
		{
			$this->query("ALTER TABLE {$table} CHANGE COLUMN {$column} {$newcolumn} {$type}{$length} NOT NULL",'alter-table-edit_sql');
		}
	...
	{
		$error = mysqli_error($this->conn['server']);
		$errorcode = mysqli_errno($this->conn['server']);
