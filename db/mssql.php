...
class DB
{
	...
	function connect($errormode = 'print') // connect to the database
	{
		if($errormode === 'return')
		{
			$ext_test = ( function_exists('mssql_connect') ? '' : -1);
			if($ext_test === -1) return -1;
			$this->conn['server'] = @mssql_connect($this->host,$this->user,$this->password);
			return (@mssql_select_db($this->database,$this->conn['server']) ? 1 : 0);
		}

		$this->conn['server'] = mssql_connect($this->host,$this->user,$this->password) or $this->error('Can\'t connect to the server');
		$this->conn['database'] = mssql_select_db($this->database,$this->conn['server']) or $this->error('Can\'t connect to the database');
	}

	function close() // close the database connection
	{
		$this->query = NULL;
		mssql_close($this->conn['server']);
	}
	...
	function query($query,$queryname,$overwrite = 'no') // run a query
	{
		...
		{
			$this->query[$queryname] = $this->currMethod = mssql_query($query,$this->conn['server']) or $this->error('Invalid query');
	...
	{
		$this->currMethod = mssql_fetch_row($this->query[$queryname]);
		return $this->currMethod;
	}
	...
	function fetch_assoc($queryname,$load = 'yes') // fetch a row as an hash
	{
		...
		{
			$this->currMethod = mssql_fetch_assoc($queryname);
			return $this->currMethod;
		}
		else
		{
			$this->currMethod = mssql_fetch_assoc($this->query[$queryname]);
			return $this->currMethod;
		}
	}
	...
	{
		$this->currMethod = mssql_num_rows($this->query[$queryname]) or $this->error('Can\'t get the number of rows from query : '.$queryname);
	...
	function showcolumns($table,$keep = 'no') // show the columns of a table
	{
		...
		while($col = $this->fetch_assoc('showcolumns_'.$table))
		{
			...
			$columns[$COLUMN_NAME]['type'] = $TYPE_NAME;
	...
	function ALTER_TABLE($table,$action,$column,$type = '',$length = '',$newcolumn = '') // add,edit,delete columns from a table
	{
		...
		{
			$this->query("ALTER TABLE {$table} DROP COLUMN {$column}",'alter-table-edit_sql','yes');
			$this->query("ALTER TABLE {$table} ADD {$newcolumn} {$type}{$length} NOT NULL",'alter-table-edit_sql');
		}
